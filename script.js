...
if (value === null || value === undefined || isNaN(value)) return "-";
return INR ${value.toLocaleString("en-IN", { maximumFractionDigits: decimals, minimumFractionDigits: decimals })};
}
...
return USD ${valueUsd.toLocaleString("en-US", { maximumFractionDigits: 1, minimumFractionDigits: 1 })};
}
...
const rawNpvDce =
  wtpAllCohorts - totalCostAllCohorts;
const rawBcrCombined =
  combinedRaw / totalCostAllCohorts;
const rawNpvCombined =
  combinedRaw - totalCostAllCohorts;
...
const natOutbreaks = results.epi.outbreaksPerYearAllCohorts || 0;
...
lines.push(
Programme cost per cohort (budgetary) is ${formatCurrency( programmeCost, state.currency )}.
);
if (state.includeOpportunityCost && opportunityCost > 0) {
lines.push(
Opportunity cost of trainee time adds ${formatCurrency( opportunityCost, state.currency )} per cohort, giving a total economic cost of ${formatCurrency( totalEconomic, state.currency )}.
);
} else {
lines.push(
Total economic cost per cohort is ${formatCurrency( totalEconomic, state.currency )}.
);
}

if (costs.templateLabel) {
lines.push(
Template used: ${costs.templateLabel}.
);
}
...
parts.push(
${tierLabel} FETP with ${cfg.traineesPerCohort} trainees per cohort and ${cfg.numberOfCohorts} planned cohorts.
);
parts.push(
Career incentive: ${cfg.career === "career_path" ? "government career pathway" : cfg.career === "uniqual" ? "university qualification" : "government and partner certificate"}.
);
parts.push(
Mentorship: ${cfg.mentorship === "high" ? "high intensity" : cfg.mentorship === "medium" ? "medium intensity" : "low intensity"}.
);
parts.push(
Delivery mode: ${cfg.delivery === "online" ? "fully online" : cfg.delivery === "inperson" ? "fully in person" : "blended"}.
);
parts.push(
Expected response time for outbreaks: detect and respond ${responseLabel}.
);

parts.push(
Cost per trainee per month is ${formatCurrency( cfg.costPerTraineePerMonth, state.currency )} with a programme duration of ${costs.durationMonths} months for this tier.
);
...
if (bcr === null || !isFinite(bcr) || results.costs.totalEconomicCostPerCohort <= 0) {
status = "neutral";
...
paragraphs.push(
${tierLabel} FETP with ${cfg.traineesPerCohort} trainees in each of ${cfg.numberOfCohorts} cohorts is predicted to attract endorsement from about ${formatPercent( endorsePct / 100, 1 )} of stakeholders in the discrete choice experiment.
);

if (typeof wtpPerTrainee === "number" && isFinite(wtpPerTrainee)) {
paragraphs.push(
The preference model implies an indicative willingness to pay of around ${formatCurrencyInr( wtpPerTrainee, 0 )} per trainee per month relative to the baseline design. Aggregated across all cohorts this corresponds to a total willingness to pay of approximately ${formatCurrencyInr( wtpAll || 0, 0 )}.
);
}

paragraphs.push(
Total economic cost across all cohorts is around ${formatCurrency( costAll || 0, state.currency )}, while the simple epidemiological module suggests total benefits of roughly ${formatCurrency( epiAll || 0, state.currency )} over the chosen planning horizon.
);

if (natBcr !== null && isFinite(natBcr)) {
paragraphs.push(
This implies a national benefit cost ratio of about ${natBcr.toFixed( 2 )}. Values above one indicate that, under the current assumptions, estimated epidemiological benefits exceed economic costs.
);
}
...
parts.push(
Under this configuration, scaling to ${cfg.numberOfCohorts} cohorts with ${cfg.traineesPerCohort} trainees each would produce around ${formatNumber( grads, 0 )} graduates once all cohorts complete.
);
parts.push(
Graduates are expected to support roughly ${formatNumber( outbreaks, 1 )} outbreak responses per year in total once the system is fully staffed.
);
parts.push(
Total economic cost of this scale up is approximately ${formatCurrency( natCost, state.currency )}, while total indicative epidemiological benefits over the planning horizon are around ${formatCurrency( natBenefit, state.currency )}.
);
if (natBcr !== null && isFinite(natBcr)) {
parts.push(
This corresponds to a national benefit cost ratio of about ${natBcr.toFixed( 2 )}, which can be compared with alternative configurations in the sensitivity tables.
);
}
...
lines.push(
Trainees per cohort: ${cfg.traineesPerCohort}, number of cohorts: ${cfg.numberOfCohorts}.
);
lines.push(
Cost per trainee per month (input): ${formatCurrencyInr( cfg.costPerTraineePerMonth, 0 )}.
);
lines.push("");
lines.push(
Planning horizon for outbreak benefits: ${s.general.planningHorizonYears} years.
);
lines.push(
Display exchange rate: ${s.general.inrPerUsd} INR per USD.
);
lines.push("");
["frontline", "intermediate", "advanced"].forEach(tier => {
const t = s.tiers[tier];
lines.push(
${tier.toUpperCase()} parameters: graduation share ${t.gradShare}, outbreak responses per cohort per year ${t.outbreaksPerCohortPerYear}, value per graduate ${formatCurrencyInr( t.valuePerGraduate, 0 )}, value per outbreak ${formatCurrencyInr( t.valuePerOutbreak, 0 )}.
);
});

lines.push("");
lines.push(
Opportunity cost included: ${state.includeOpportunityCost ? "yes" : "no"}.
);
...
const cellsHead = [
  ...
  formatPercent(endorseUsedPct / 100, 1),
  effectiveWtp !== null
...
doc.setFontSize(9);
doc.text(
Benefit definition view: ${benefitDefinition}, outbreak benefits included: ${ includeEpi ? "yes" : "no" }.,
40,
60
);
...
doc.setFontSize(9);
doc.text(
  `Endorsement used: ${(profile.endorsementUsed * 100).toFixed(
    1
  )} %, cost (all cohorts): ${formatCurrencyInr(
    profile.totalCostAllCohorts || 0,
    0
  )}.`,
  40,
  y
...
doc.text(
  `Total WTP: ${formatCurrencyInr(
    profile.wtpAllCohorts || 0,
    0
  )}, WTP from response: ${formatCurrencyInr(
    profile.wtpRespAllCohorts || 0,
    0
  )}, epi outbreak benefit: ${formatCurrencyInr(
    profile.epiOutbreakAllCohorts || 0,
    0
  )}.`,
  40,
...
doc.setFontSize(9);
doc.text(
  `Tier: ${sc.config.tier}, mentorship: ${sc.config.mentorship}, delivery: ${sc.config.delivery}, response: ${sc.config.response} days.`,
  40,
  y
);
y += 11;
...
doc.text(
  `Cohorts: ${sc.config.numberOfCohorts}, trainees per cohort: ${sc.config.traineesPerCohort}, cost per trainee per month: ${formatCurrencyInr(
    sc.config.costPerTraineePerMonth,
    0
  )}.`,
  40,
...
doc.text(
  `Endorsement: ${formatPercent(
    sc.endorsementRate,
    1
  )}, total economic cost: ${formatCurrencyInr(
    sc.totalCostAllCohorts || 0,
    0
  )}, total epidemiological benefit: ${formatCurrencyInr(
    sc.totalEpiBenefitAllCohorts || 0,
    0
  )}.`,
  40,
...
lines.push(<p><strong>Scenario name:</strong> ${ cfg.scenarioName || "Not specified" }</p>);
lines.push(
<p><strong>Programme tier:</strong> ${cfg.tier}, <strong>career incentive:</strong> ${cfg.career}, <strong>mentorship:</strong> ${cfg.mentorship}, <strong>delivery:</strong> ${cfg.delivery}, <strong>response time:</strong> ${cfg.response} days.</p>
);
lines.push(
<p><strong>Cohorts and intake:</strong> ${cfg.numberOfCohorts} cohorts with ${cfg.traineesPerCohort} trainees per cohort.</p>
);
lines.push(
<p><strong>Endorsement:</strong> ${formatPercent( results.util.endorseProb || 0, 1 )}; <strong>opt out:</strong> ${formatPercent( results.util.optOutProb || 0, 1 )}.</p>
);
lines.push(
<p><strong>Total economic cost (all cohorts):</strong> ${formatCurrency( results.totalCostAllCohorts || 0, state.currency )}; <strong>total epidemiological benefit:</strong> ${formatCurrency( results.epi.totalBenefitAllCohorts || 0, state.currency )}.</p>
);
if (results.natBcr !== null && isFinite(results.natBcr)) {
lines.push(
<p><strong>National benefit cost ratio:</strong> ${results.natBcr.toFixed( 2 )}; <strong>national net benefit:</strong> ${formatCurrency( results.natNetBenefit || 0, state.currency )}.</p>
);
}

if (cfg.scenarioNotes) {
lines.push(
<p><strong>Scenario notes:</strong> ${cfg.scenarioNotes}</p>
);
}
